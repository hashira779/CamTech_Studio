...
def tokenize_line_words(text: str) -> List[str]:
    """
    Splits text into words. Uses khmernltk for Khmer script (since Khmer
    has no word-delimiter spaces), and whitespace splitting for Latin script.
    """
    if not text:
        return []
    ...
    if not clean:
        return []
    if is_khmer_text(clean) and khmer_word_tokenize:
        try:
...
def find_matching_subtitles(audio_path: str) -> Optional[str]:
    """
    Searches for subtitle files (.vtt, .srt, .lrc) that match an audio file.
    Prioritizes Khmer (.km.vtt, .km.srt) and local subtitle files.
    """
    if not audio_path:
        return None
    ...
    for d in search_dirs:
        ...
        for f in os.listdir(d):
            ...
            if base_name in f or f_base.startswith(base_name) or base_name.startswith(f_base):
                ...
                priority = 0
                if ".km." in f_lower or "khmer" in f_lower:
                    priority += 15
                if f_lower.endswith(".vtt"):
                    priority += 4
...
class WhisperTranscriber:
    ...
    def transcribe(self, audio_path: str, language: Optional[str] = None, progress_callback=None) -> List[Dict[str, Any]]:
        ...
        # If language is unspecified, check if audio filename has Khmer characters
        if language is None and is_khmer_text(os.path.basename(audio_path)):
            language = "km"
            ...
            for segment in segments:
                ...
                if segment.words:
                    for w in segment.words:
                        ...
                        # Apply Khmer tokenization
                        if is_khmer_text(w_text) and khmer_word_tokenize:
